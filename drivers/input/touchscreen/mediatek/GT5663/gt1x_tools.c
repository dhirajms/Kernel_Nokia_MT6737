...
static ssize_t gt1x_tool_read(struct file *filp, char __user * buffer, size_t count, loff_t * ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		*ppos += cmd_head.data_len;
		...
		u8 temp_buff[4] = {0};
		temp_buff[0] = update_info.progress >> 8;
		temp_buff[1] = update_info.progress & 0xff;
		temp_buff[2] = update_info.max_progress >> 8;
		temp_buff[3] = update_info.max_progress & 0xff;
		if (copy_to_user(buffer, temp_buff, 4)) {
			GTP_ERROR("[READ]copy_to_user failed.");
			return -1;
		}
		*ppos += 4;
		...
		if (copy_to_user(&buffer[tmp_len], &end_mark, 1)) {
			GTP_ERROR("[READ]copy_to_user failed.");
			return -1;
		}
		*ppos += tmp_len + 1;
