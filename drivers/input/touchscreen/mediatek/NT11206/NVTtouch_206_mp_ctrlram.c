 ...
 *
 * $Revision: 6747 $
 * $Date: 2016-10-11 16:45:37 +0800 (週二, 11 十月 2016) $
 *
...
int32_t PSConfig_Tolerance_Postive_Mutual = 300;
int32_t PSConfig_Tolerance_Negative_Mutual = -300;
int32_t PSConfig_DiffLimitG_Postive_Mutual = 300;
int32_t PSConfig_DiffLimitG_Negative_Mutual = -300;
...
static int32_t nvt_read_short_rxrx(void)
{
	...
	char ctrlram_short_rxrx_file[64]="/system/etc/nvt_firmware/CtrlRAM_Short_RXRX.csv";
	char ctrlram_short_rxrx1_file[64]="/system/etc/nvt_firmware/CtrlRAM_Short_RXRX1.csv";
...
static int32_t nvt_read_short_txrx(void)
{
	...
	int32_t write_ret = 0;
	uint32_t output_len = 0;
	char ctrlram_short_txrx_file[64]="/system/etc/nvt_firmware/CtrlRAM_Short_TXRX.csv";
...
static int32_t nvt_read_short_txtx(void)
{
	...
	int32_t write_ret = 0;
	uint32_t output_len = 0;
	char ctrlram_short_txtx_file[64]="/system/etc/nvt_firmware/CtrlRAM_Short_TXTX.csv";
...
static int32_t nvt_read_open(void)
{
	...
	int32_t write_ret = 0;
	uint32_t output_len = 0;
	char ctrlram_open_mutual_file[64]="/system/etc/nvt_firmware/CtrlRAM_Open_Mutual.csv";
...
int nvt_selftest(void)
{
...
#if TOUCH_KEY_NUM > 0
		TestResult_Short_TXRX_Key = RawDataTest_Sub(BoundaryShort_TXRX_Key, TXRX_test_keydata, RecordResultShort_TXRX_Key, Key_X_Channel_Real, Key_Y_Channel_Real,
												PSConfig_Tolerance_Postive_Key_Short, PSConfig_Tolerance_Negative_Key_Short, PSConfig_DiffLimitG_Postive_Key_Short, PSConfig_DiffLimitG_Negative_Key_Short,
   ...
   printk(" selftest value %d %d %d %d\n",TestResult_Short_RXRX,TestResult_Short_TXRX,TestResult_Short_TXTX,TestResult_Open);

   
   printk(" selftest value %d %d %d %d\n",TestResult_Short_RXRX_Key,TestResult_Short_TXRX_Key,TestResult_Short_TXTX_Key,TestResult_Open_Key);
	...
	printk(" nvt_selftest end\n");
    printk(" tp selftest result = %d\n", ret);
...
//add by alex for selftest
static ssize_t nvt_sysfs_tp_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  //  ssize_t ret = 0;
    ssize_t len = 0;
    nvt_get_fw_info();
    len += sprintf(&(buf[len]), "NVTtouch_206 V%x\n", fw_ver);

    return len;
}


static ssize_t nvt_sysfs_tp_rawdata_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  
    ssize_t len = 0;
   len += sprintf(&(buf[len]), "Data=2;(D,%x,%x,%x,%x);",PSConfig_Tolerance_Postive_Mutual,PSConfig_Tolerance_Negative_Mutual,PSConfig_Rawdata_Limit_Postive_Short_RXRX,PSConfig_Rawdata_Limit_Negative_Short_RXRX);
	return len;
}
...
static ssize_t nvt_sysfs_selftest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    ssize_t ret = 0;
    ssize_t len = 0;
...
static ssize_t nvt_sysfs_selftest_fqc(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    ssize_t ret = 1;
...
static ssize_t nvt_get_selftest_result(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 1;
  	ssize_t len = 0;
...
static DEVICE_ATTR(ftmgetversion, S_IRUGO|S_IWUSR, nvt_sysfs_tp_version_show, NULL);
static DEVICE_ATTR(tp_rawdata, S_IRUGO|S_IWUSR, nvt_sysfs_tp_rawdata_show, NULL);
static DEVICE_ATTR(fts_selftest, S_IRUGO|S_IWUSR, nvt_sysfs_selftest_show, nvt_sysfs_selftest_fqc);
static DEVICE_ATTR(fts_selftest_result, S_IRUGO|S_IWUSR, nvt_get_selftest_result, NULL);
