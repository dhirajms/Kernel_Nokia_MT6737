 ...
 *
 * $Revision: 4056 $
 * $Date: 2016-04-07 10:02:17 +0800 (週四, 07 四月 2016) $
 *
...
#include "NVTtouch_206firmware_E1.h"
...
int32_t Check_FW_Ver(void)
{
    ...
    #if(BOOT_UPDATE_HeaderFile)	
    dev_info(&ts->client->dev, "Bin@HeaderFile FW Ver = 0x%02X, FW ver Bar = 0x%02X\n",
	...
	#if(BOOT_UPDATE_HeaderFile)	
	if (buf[1] == BUFFER_DATA[FW_BIN_VER_OFFSET])
	#else
...
int32_t Erase_Flash(void)
{
	...
	int32_t retry = 0;
    #if(BOOT_UPDATE_HeaderFile)
	if (BUFFER_LENGTH % FLASH_SECTOR_SIZE)
		count = BUFFER_LENGTH / FLASH_SECTOR_SIZE + 1;
	else
		count = BUFFER_LENGTH / FLASH_SECTOR_SIZE;
    #else
...
int32_t Write_Flash(void)
{
	...
	int32_t retry = 0;
	#if(BOOT_UPDATE_HeaderFile)
	size_t fw_bin_size = BUFFER_LENGTH; 
    #endif
	...
	if (BUFFER_LENGTH % 256)
		count = BUFFER_LENGTH / 256 + 1;
	else
		count = BUFFER_LENGTH / 256;
	#else	
	...
	for (i = 0; i < count; i++) {
		...
		#if(BOOT_UPDATE_HeaderFile)	//Faster//[20161129,jx]Modify for non-factor of 256
		{
			int32_t step = 55;
		
			int32_t restSize = min(fw_bin_size - i * 256, (size_t)256);
		    for (j = 0; j < restSize; j += step) {
				...
				for (k = 0; k < i32tail; k++) {				
					buf[1 + k] = BUFFER_DATA[Flash_Address + j + k];
				}
			...
			if (BUFFER_LENGTH - Flash_Address >= 256)
			tmpvalue=(Flash_Address >> 16) + ((Flash_Address >> 8) & 0xFF) + (Flash_Address & 0xFF) + 0x00 + (255);
		else
			tmpvalue=(Flash_Address >> 16) + ((Flash_Address >> 8) & 0xFF) + (Flash_Address & 0xFF) + 0x00 + (BUFFER_LENGTH - Flash_Address - 1);

		for (k = 0;k < min(fw_bin_size - Flash_Address,(size_t)256); k++)
		...
		buf[5] = 0x00;
		#if(BOOT_UPDATE_HeaderFile)
		buf[6] = min(fw_bin_size - Flash_Address,(size_t)256) - 1;
		#else
