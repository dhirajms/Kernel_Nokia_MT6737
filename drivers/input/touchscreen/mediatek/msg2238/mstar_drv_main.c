...
static u8 *fqcgetversion = NULL;//add by yangnan for read fw version
...
/*add by yangnan for read fw version and tp selftest start*/
extern  int msg2238_SelfTest(void);

static struct kobject *fih_tptest_kobj;
static int rawDefMin,rawDefMax,rawTestVal,rawTestRow,rawTestColumn;


static ssize_t fih_sysfs_tp_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{    
	u32 nLength = 0;
	u16 nMajor = 0, nMinor = 0;

    DrvIcFwLyrGetCustomerFirmwareVersion(&nMajor, &nMinor, &fqcgetversion);
	DBG("*** %s() fqcgetversion = %s ***\n", __func__, fqcgetversion);
...
#if 1  //alex
static ssize_t fih_sysfs_selftest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    int ret= 0;

	//Added by CNTouch

	ret = msg2238_SelfTest();
...
static ssize_t fih_sysfs_selftest_fqc(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    /*int ret;
	ret = msg2238_SelfTest();
...
static ssize_t fih_get_selftest_result(struct device *dev,struct device_attribute *attr, char *buf)
{    
	int ret = 0;	
...
static ssize_t fih_sysfs_tp_ftmcalibration_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	 int len = 0;
...
static ssize_t fih_sysfs_tp_rawdata_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
	rawDefMin = 0; rawDefMax = 0; rawTestVal = 0; rawTestRow = 0; rawTestColumn = 0;
...
static DEVICE_ATTR(ftmgetversion, S_IRUGO|S_IWUSR, fih_sysfs_tp_version_show, NULL);//add by yangnan for read fw version
//static DEVICE_ATTR(fts_selftest, S_IRUGO|S_IWUSR, fih_sysfs_selftest_show, fih_sysfs_selftest_fqc);
static DEVICE_ATTR(fts_selftest, S_IRUSR|S_IWUSR/*|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH*/, fih_sysfs_selftest_show, fih_sysfs_selftest_fqc);
static DEVICE_ATTR(fts_selftest_result, S_IRUGO|S_IWUSR, fih_get_selftest_result, NULL);
static DEVICE_ATTR(ftmcalibration, S_IRUGO|S_IWUSR, fih_sysfs_tp_ftmcalibration_show, NULL);//add by yangnan for get ftmcalibration
static DEVICE_ATTR(tp_rawdata, S_IRUGO|S_IWUSR, fih_sysfs_tp_rawdata_show, NULL);//add by yangnan for get tp_rawdata





int fih_tptest_sysfs_init(void)
{    
	int ret;	    
