...
/*open short test start add by yangnan.*/
...
//Added by CNTouch
int msg2238_SelfTest(void)
{
	   int retval=-1;
	   int TP6206_count=0;
	   char cmd_buf[4]={0xff,0x55,0xff,0x66};
	   char buf[8];
	   int try_cou ;
	   
	   for(try_cou = 0 ; try_cou<15;try_cou++)
	   {	   
          IicWriteData(SLAVE_I2C_ID_DWI2C, &cmd_buf[0], 4);
	   	  mdelay(1200);  // Delay 800 m sec
	   	  printk("bobo1==TEST1,try_cou=%d.\n",try_cou);
		
	      TP6206_count = 0 ;
	   	  while(TP6206_count<50)
	  	  {
			retval = IicReadData(SLAVE_I2C_ID_DWI2C, &buf[0], 8);

	     		// printk("bobo2==%d\n",buf[0]);
     			if((buf[0]==0x55)||(buf[0]==0x54))
     			{
     				break;
     			}
	     		// mdelay(500);
     			mdelay(10);  // Delay 10 m sec
     			TP6206_count++;
      		} // End while

	  	  DrvPlatformLyrTouchDeviceResetHw();
		  mdelay(800);  // Delay 500 m sec

	  	  printk("BOBO6206 TP TEST the value is %d,TP6206_count=%d.\n",buf[0],TP6206_count);
      	  if(buf[0]==0x55)
      	  {
     	      return 0;
      	  }
      	  else if(buf[0]==0x54)
      	  {
     		...
     		{}   // Don't have response
     	} // End for(try_cou = 0 ; try_cou<15;try_cou++)
     //tpd_hw_enable(1);
     return 4;
}
