...
static ssize_t kpd_reboot_show(struct device_driver *ddri, char *buf)
{
    ...
    pmic_read_interface(MT6328_TOP_RST_MISC, &pwrkey_rst, MT6328_PMIC_RG_PWRKEY_RST_EN_MASK, MT6328_PMIC_RG_PWRKEY_RST_EN_SHIFT);
    pmic_read_interface(MT6328_TOP_RST_MISC, &homekey_rst, MT6328_PMIC_RG_HOMEKEY_RST_EN_MASK, MT6328_PMIC_RG_HOMEKEY_RST_EN_SHIFT);

    if (pwrkey_rst || homekey_rst)
	{
        pmic_read_interface(MT6328_TOP_RST_MISC, &reboot_time, MT6328_PMIC_RG_PWRKEY_RST_TD_MASK, MT6328_PMIC_RG_PWRKEY_RST_TD_SHIFT);
        switch (reboot_time)
        {
            case 3:
                reboot_time = 5;
...
static ssize_t kpd_reboot_store(struct device_driver *ddri, const char *buf, size_t count)
{
    int reboot_time = 0;

	if (sscanf(buf, "%u", &reboot_time) != 1) {
   		return -EINVAL;
	}
