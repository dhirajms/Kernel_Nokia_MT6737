...
void kpd_slide_qwerty_init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek, kpd_slide-eint");
		if (node) {
			...
			kpd_slide_irq = irq_of_parse_and_map(node, 0);
            
			kpd_print(" kpd_slide_irq value = %d\n",kpd_slide_irq);
			
			ret = request_irq(kpd_slide_irq, (irq_handler_t) kpd_slide_eint_handler, IRQ_TYPE_EDGE_BOTH,
					"KPD_SLIDE-eint", NULL);
			if (ret > 0)
				kpd_print("EINT IRQ LINEN NOT AVAILABLE\n");			
		} else
