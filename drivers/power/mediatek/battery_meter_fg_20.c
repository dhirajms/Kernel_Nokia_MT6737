...
#endif
bool charger_detect_flag = false;
...
static int __batt_meter_init_cust_data_from_cust_header(void)
{
	...
	batt_meter_cust_data.bat_full_chk_thr = BAT_FULL_CHK_THR;
	batt_meter_cust_data.shutdown_gauge0_voltage = SHUTDOWN_GAUGE0_VOLTAGE;
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	while (!of_property_read_u32_index(np, "batt_temperature_table", idx, &addr)) {
		...
		// test Batt_Temperature_Table[idx / 2].BatteryTemp = addr;
		// test Batt_Temperature_Table[idx / 2].TemperatureR = val;
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_RTC:
		{
			signed int rtcvalue;
			static signed int previous_value = -1;
