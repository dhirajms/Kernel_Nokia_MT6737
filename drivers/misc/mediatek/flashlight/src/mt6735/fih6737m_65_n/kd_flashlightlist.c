...
struct pinctrl_state *flash_pwm_pwm = NULL;
...
int mtkflash_gpio_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(flashctrl)) {
		...
		ret = PTR_ERR(flashctrl);
			...
			flash_pwm_pwm = pinctrl_lookup_state(flashctrl, "flash1_state2");
			if (IS_ERR(flash_pwm_pwm)) {
				ret = PTR_ERR(flash_pwm_pwm);
...
int mtkflash_gpio_set(int pin, int mod)
{
	...
	if (pin == FLASH_EN){
		...
		if (mod == PIN_HIGH)
            ...
            pinctrl_select_state(flashctrl, flash_pwm_pwm);
...
static int flashlight_open(struct inode *inode, struct file *file)
{
	...
	//#if !defined(CONFIG_MTK_LEGACY)
	//mtkflash_gpio_init(flash_plt_dev);
	//mtkflash_gpio_set(0, 1);
    //#endif
...
/**ZL3 add for flash proc control start */
#ifdef FLASH_PROC_CTL
    proc_create("driver/flashlight", 0, NULL, &flash_proc_fops);
#endif
/**ZL3 add for flash proc control end */
/**ZL3 add for sub flash proc control start */
#ifdef SUB_FLASH_PROC_CTL
    proc_create("driver/subflashlight", 0, NULL, &sub_flash_proc_fops);
#endif
/**ZL3 add for sub flash proc control end */
#ifndef CONFIG_OF
