...
/*for jingkang start*/
static int mcjk_write(struct seq_file *s, void *unused)
{
	seq_printf(s,"Samsung S5K4H8(8M JK)(AF)(Non-SOC)(RAW)(MIPI)\n");
	return 0;
}
static int mcjk_open(struct inode *inode, struct file *file)
{
	return single_open(file, mcjk_write, &inode->i_private);
}
static const struct file_operations mcjk_fops = {
	.owner 		 = THIS_MODULE,	
    .open        = mcjk_open,
    .read        = seq_read,
    .llseek      = seq_lseek,
...
#define S5K4H8_OTP_ENABLE 1
#if S5K4H8_OTP_ENABLE
extern int s5k4h8_read_otp(u16 offset, u8* data, u32 size);

typedef struct _s5k4h8_otp_data
{
...
static s5k4h8_otp_data g_otpdata = {0,};
...
int s5k4h8_applyawb(s5k4h8_otp_data *pdata)
{
    ...
    int R_gain,B_gain,Gb_gain,Gr_gain,Base_gain;
                                 // 
    LOG_INF("[s5k4h8otp]OTP WB Update Start: \n"); 
    ...
    LOG_INF("[s5k4h8otp]OTP WB Update Finished! \n");    
    return TRUE;
}

static int s5k4h8_applyLSC(void)
{
	int i=0,reg1=0,reg2=0;
	LOG_INF("[s5k4h8otp]OTP LSC Update Start: \n"); 
	...
	if(i>=5){
		LOG_INF("[s5k4h8otp]OTP LSC Update failed! reg1=%d,reg2=%d,i=%d\n",reg1,reg2,i);
		return -1;
	}
	LOG_INF("[s5k4h8otp]OTP LSC  reg1=%d,reg2=%d,i=%d\n",reg1,reg2,i);
	LOG_INF("[s5k4h8otp]OTP LSC Update Finished! \n");
	mdelay(5);
...
#define S5K4H8JK_OTP_ENABLE 1

#if S5K4H8JK_OTP_ENABLE
extern int s5k4h8jk_read_otp(u16 offset, u8* data, u32 size);

typedef struct _s5k4h8jk_otp_data
{
    u8   infoflag;//valid info&awb group
    u8   module_integrator_id;
    u8   R_G_H;
...
static s5k4h8jk_otp_data g_jkotpdata = {0,};
static u8              g_jkotpread = 0;

#define MCJK_REG_PROC "MainjkCamReg"
static int mcjk_reg_write(struct seq_file *s,void *unused)
{
    char  sId[512] = {0,};
...
static int mcjk_reg_open(struct inode *inode, struct file *file)
{
	return single_open(file, mcjk_reg_write, &inode->i_private);
}

static const struct file_operations mcjk_reg_fops = {
    .owner 		 = THIS_MODULE,
    .open        = mcjk_reg_open,
    .read        = seq_read,
    .llseek      = seq_lseek,
...
int s5k4h8jk_applyawb(s5k4h8jk_otp_data *pdata)
{
       ...
       /* R_ratio,B_ratio parameters Multiply 100 times */
    ...
    LOG_INF("[s5k4h8jkotp]OTP WB Update Finished! \n");
    return TRUE;
}


static int s5k4h8jk_applyLSC(void)
{
    write_cmos_sensor(0x0B00, 0x0180);
    write_cmos_sensor(0x6028, 0x2000);
    write_cmos_sensor(0x602A, 0x0C40);
...
LOG_INF("[get_imgsensor_id]camIdPin ==  %d \n", camIdPin);
    ...
    if(i == 1){
   	#if S5K4H8_OTP_ENABLE
    LOG_INF("[s5k4h8otp]read otp start \n");
    if( !g_otpread )
    {
		//write_cmos_sensor(0x0100,0x01);
        if( 0 != s5k4h8_read_otp(0, (u8*)&g_otpdata, sizeof(g_otpdata)) )
        {
            LOG_INF("[s5k4h8otp]otp read fail!\n");
            *sensor_id = 0xFFFFFFFF;
            return ERROR_SENSOR_CONNECT_FAIL;
        }
        g_otpread = 1;
    }
    LOG_INF("[s5k4h8otp]g_otpdata.module_integrator_id:0x%x, g_otpdata.lens_vcm_id:0x%x, "
	...
	{
        LOG_INF("[s5k4h8otp]otp data error!\n");
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
	}
...
#if S5K4H8JK_OTP_ENABLE
            LOG_INF("[s5k4h8jkotp]read otp start \n");
            if( !g_jkotpread )
            {
                //write_cmos_sensor(0x0100,0x01);
                if( 0 != s5k4h8jk_read_otp(0, (u8*)&g_jkotpdata, sizeof(g_jkotpdata)) )
                {
                    LOG_INF("[s5k4h8jkotp]otp read fail!\n");
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }
                g_jkotpread = 1;
            }
            LOG_INF("[s5k4h8jkotp]g_otpdata.module_integrator_id:0x%x, g_jkotpdata.lens_vcm_id:0x%x, ",
    ...
    LOG_INF("vcm_inf_H:0x%x,vcm_inf_L:0x%x vcm_mac_H : 0x%x,vcm_mac_L : 0x%x\n",
            ...
            {
                LOG_INF("[s5k4h8jkotp]otp data error!\n");
                *sensor_id = 0xFFFFFFFF;
                return ERROR_SENSOR_CONNECT_FAIL;
            }

            /**Tarry add for write module info to file start  */
            regjk_entry = proc_create(MCJK_REG_PROC, S_IFREG | S_IRWXUGO, NULL,&mcjk_reg_fops);
            if (regjk_entry == NULL)
            {
                LOG_INF("[s5k4h8otp]creat proc %s fail!\n", MCJK_REG_PROC);
            }
	...
	entryjk = proc_create(MC_PROC, S_IFREG | S_IRWXUGO, NULL,&mcjk_fops);
	//entry = create_proc_entry(FC_PROC, S_IFREG | S_IRWXUGO, NULL);
	if (entryjk == NULL)
	{
		LOG_INF("[s5k4h8]creat proc %s fail!\n", MC_PROC);
...
if(i == 1){
	#if S5K4H8_OTP_ENABLE
    s5k4h8_applyawb(&g_otpdata);
	s5k4h8_applyLSC();
    #endif
}else if(i == 2){
#if S5K4H8JK_OTP_ENABLE
    s5k4h8jk_applyawb(&g_jkotpdata);
	s5k4h8jk_applyLSC();
#endif
    }
	spin_lock(&imgsensor_drv_lock);
