...
#include "ovs5k4h8submipi_Sensor.h"

#define PFX "OVS5K4H8SUB_camera_sensor"
#define LOG_1 LOG_INF("OVS5K4H8SUB,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OVS5K4H8SUB_SENSOR_ID,
...
#define OVS5K4H8SUB_OTP_ENABLE 1
#if OVS5K4H8SUB_OTP_ENABLE
extern int s5k4h8sub_read_otp(u16 offset, u8* data, u32 size);

typedef struct _ovs5k4h8sub_otp_data
{
    ...
    u16  vcm_inf;
    u16  vcm_mac;
}ovs5k4h8sub_otp_data;

static ovs5k4h8sub_otp_data g_otpdata = {0,};
...
/*
int ovs5k4h8sub_applyawb(ovs5k4h8sub_otp_data *pdata)
{
    ...
    int R_gain,B_gain,Gb_gain,Gr_gain,Base_gain;
                                 //
    LOG_INF("[ovs5k4h8subotp]OTP WB Update Start: \n");
    ...
    LOG_INF("[ovs5k4h8subotp]OTP WB Update Finished! \n");
    return TRUE;
}
*/

int  S5K4H8SUB_read_ofilm_otp(ovs5k4h8sub_otp_data *otp_ptr)
{
 ...
 //set OTP page
   ...
   infoflag= read_cmos_sensor(0x0A04);
	 ...
	 LOG_INF("----hejun ofilm  S5K4H8SUB_read_otp() addr=%x\n",addr);
	 if(addr!=0)
	 {
     ...
     (*otp_ptr).wb_rg_ratio = (((read_cmos_sensor(0x0A0F) << 2)&0x03FC) + ((temp & (0x30))>>4));	     
	  ...
	  LOG_INF("----hejun ofilm  S5K4H8SUB_read_otp() otp_flag=%x\n",infoflag);
	  LOG_INF("----hejun ofilm  S5K4H8SUB_read_otp() rg_ratio=%x\n",(*otp_ptr).wb_rg_ratio);
	  LOG_INF("----hejun ofilm  S5K4H8SUB_read_otp() bg_ratio=%x\n",(*otp_ptr).wb_bg_ratio);
 
	  return 0;
}

#define GAIN_DEFAULT 0x0100
#define  S5K4H8SUB_RG_Ratio_Typical  0x134  //????????????
#define  S5K4H8SUB_BG_Ratio_Typical  0x158  //????????????	
int S5K4H8SUB_OFilm_apply_otp(void)
{
  ovs5k4h8sub_otp_data current_otp;
  ...
  Gr_GAIN = 0;
   ...
   S5K4H8SUB_read_ofilm_otp(&current_otp);
  ...
  if(current_otp.infoflag==0xC0)
  {
    ...
    bg=current_otp.wb_bg_ratio;
		...
		r_ratio = 512 * ( S5K4H8SUB_RG_Ratio_Typical) /(rg);
		b_ratio = 512 * ( S5K4H8SUB_BG_Ratio_Typical) /(bg);
	...
	if(B_GAIN>0x0100)
		...
		LOG_INF("----hejun ofilm  S5K4H8SUB_apply_otp() R_GAIN=%x,B_GAIN=%x,G_GAIN=%x\n",R_GAIN,B_GAIN,G_GAIN);
		/*
...
LOG_INF("----hejun ofilm  S5K4H8SUB_apply_otp() R_GAIN=%x,R_GAIN=%x\n",gain_01,gain_02);
LOG_INF("----hejun ofilm  S5K4H8SUB_apply_otp() G_GAIN=%x,G_GAIN=%x\n",gain_03,gain_04);
LOG_INF("----hejun ofilm  S5K4H8SUB_apply_otp() B_GAIN=%x,B_GAIN=%x\n",gain_05,gain_06);
LOG_INF("----hejun ofilm  S5K4H8SUB_apply_otp() Gr_GAIN=%x,Gb_GAIN=%x\n",gain_07,gain_08);
		*/
...
#if 0
static int ovs5k4h8sub_applyLSC(void)
{
	int i=0,reg1=0,reg2=0;
	LOG_INF("[ovs5k4h8subotp]OTP LSC Update Start: \n");
	...
	if(i>=5){
		LOG_INF("[ovs5k4h8subotp]OTP LSC Update failed! reg1=%d,reg2=%d,i=%d\n",reg1,reg2,i);
		return -1;
	}
	LOG_INF("[ovs5k4h8subotp]OTP LSC  reg1=%d,reg2=%d,i=%d\n",reg1,reg2,i);
	LOG_INF("[ovs5k4h8subotp]OTP LSC Update Finished! \n");
	mdelay(5);
...
LOG_INF("[get_imgsensor_id]cam1IdPin ==  %d \n", cam1IdPin);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id += 3;
			printk("ovs5k4h8sub get_imgsensor_id sensor_id=%x\r\n",*sensor_id);
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
   	...
   	#if OVS5K4H8SUB_OTP_ENABLE
    LOG_INF("[ovs5k4h8subotp]read otp start \n");
    if( !g_otpread )
    {
		//write_cmos_sensor(0x0100,0x01);
        if( 0 != s5k4h8sub_read_otp(0, (u8*)&g_otpdata, sizeof(g_otpdata)) )
        {
            LOG_INF("[ovs5k4h8subotp]otp read fail!\n");
            *sensor_id = 0xFFFFFFFF;
            return ERROR_SENSOR_CONNECT_FAIL;
        }
        g_otpread = 1;
    }
    LOG_INF("[ovs5k4h8subotp]g_otpdata.module_integrator_id:0x%x, g_otpdata.lens_vcm_id:0x%x, "
	...
	{
        LOG_INF("[ovs5k4h8subotp]otp data error!\n");
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
	}
	...
	if (entry == NULL)
	{
		LOG_INF("[ovs5k4h8sub]creat proc %s fail!\n", FC_PROC);
	}
...
UINT32 OVS5K4H8SUB_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	ovs5k4h8sub_MIPI_RAW_SensorInit	*/
/*
	 20160107 end
*/