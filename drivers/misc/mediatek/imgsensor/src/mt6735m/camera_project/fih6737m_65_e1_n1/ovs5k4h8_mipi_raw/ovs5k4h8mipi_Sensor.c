...
#include "ovs5k4h8mipi_Sensor.h"

#define PFX "OVS5K4H8_camera_sensor"
#define LOG_1 LOG_INF("OVS5K4H8,MIPI 4LANE\n")
...
#define MC_PROC "MainCam"
static int mc_write(struct seq_file *s, void *unused)
{
	seq_printf(s,"Samsung OVS5K4H8(8M)(AF)(Non-SOC)(RAW)(MIPI)\n");
	return 0;
}
static int mc_open(struct inode *inode, struct file *file)
{
	return single_open(file, mc_write, &inode->i_private);
}
...
#define OVS5K4H8_OTP_ENABLE 1
#if OVS5K4H8_OTP_ENABLE
extern int s5k4h8_read_otp(u16 offset, u8* data, u32 size);

typedef struct _ovs5k4h8_otp_data
{
...
static ovs5k4h8_otp_data g_otpdata = {0,};
static u8              g_otpread = 0;

#define MC_REG_PROC "MainCamReg"
static int mc_reg_write(struct seq_file *s,void *unused)
{
    char  sId[512] = {0,};
...
static int mc_reg_open(struct inode *inode, struct file *file)
{
	return single_open(file, mc_reg_write, &inode->i_private);
}

static const struct file_operations mc_reg_fops = {
    .owner 		 = THIS_MODULE,
    .open        = mc_reg_open,
    .read        = seq_read,
    .llseek      = seq_lseek,
...
/*
int ovs5k4h8_applyawb(ovs5k4h8_otp_data *pdata)
{
    ...
    int R_gain,B_gain,Gb_gain,Gr_gain,Base_gain;
                                 //
    LOG_INF("[ovs5k4h8otp]OTP WB Update Start: \n");
    ...
    LOG_INF("[ovs5k4h8otp]OTP WB Update Finished! \n");
    return TRUE;
}
*/

int  S5K4H8_read_ofilm_otp(ovs5k4h8_otp_data *otp_ptr)
{
   int infoflag,addr,i, page, temp;
	 addr=0;
	 i=0;
	 page = 15;
	 temp = 0 ;
 ...
 //set OTP page
   ...
   write_cmos_sensor_8(0x0A00, 0x01);   //Enter read mode by writing 01h to 

   infoflag= read_cmos_sensor(0x0A04);
	 if(infoflag==0x40)         
	 {
	    addr=0x0A05; //group 1
	    i=1;
	 }
	 else if((infoflag==0xD0))    
	 {
	    addr=0x0A0C; //group 2
	    i=2;
	 }
	 LOG_INF("----hejun ofilm  S5K4H8_read_otp() addr=%x\n",addr);
	 if(addr!=0)
	 {
     (*otp_ptr).infoflag = 0xc0;
	 ...
	 {
     (*otp_ptr).infoflag = 0;
     (*otp_ptr).lens_vcm_id = 0;
     (*otp_ptr).wb_rg_ratio = 0;
     (*otp_ptr).wb_bg_ratio = 0;
	 }
	 
	   if(i==1)
     {
     //OTP WB Calibration Data
     temp = read_cmos_sensor(0x0A0A);
     (*otp_ptr).wb_rg_ratio = (((read_cmos_sensor(0x0A07) << 2)&0x03FC) + ((temp & (0xC0))>>6));
     (*otp_ptr).wb_bg_ratio = (((read_cmos_sensor(0x0A08) << 2)&0x03FC) + ((temp & (0x30))>>4));	 
     }
     else if(i==2)
     {
     //OTP WB Calibration Data
     temp = read_cmos_sensor(0x0A11);
     (*otp_ptr).wb_rg_ratio = (((read_cmos_sensor(0x0A0E) << 2)&0x03FC) + ((temp & (0xC0))>>6));
     (*otp_ptr).wb_rg_ratio = (((read_cmos_sensor(0x0A0F) << 2)&0x03FC) + ((temp & (0x30))>>4));	     
     }  
     	  
	  LOG_INF("----hejun ofilm  S5K4H8_read_otp() otp_flag=%x\n",infoflag);
	  LOG_INF("----hejun ofilm  S5K4H8_read_otp() rg_ratio=%x\n",(*otp_ptr).wb_rg_ratio);
	  LOG_INF("----hejun ofilm  S5K4H8_read_otp() bg_ratio=%x\n",(*otp_ptr).wb_bg_ratio);
 
	  return 0;
}

#define GAIN_DEFAULT 0x0100
#define  S5K4H8_RG_Ratio_Typical  0x134  //????????????
#define  S5K4H8_BG_Ratio_Typical  0x158  //????????????	
int S5K4H8_OFilm_apply_otp(void)
{
  ovs5k4h8_otp_data current_otp;
  int rg,bg,r_ratio,b_ratio,R_GAIN,B_GAIN,G_GAIN,Gb_GAIN,Gr_GAIN;
  R_GAIN = 0;
  B_GAIN = 0;
  G_GAIN = 0;
  Gb_GAIN = 0;
  Gr_GAIN = 0;
   ...
   S5K4H8_read_ofilm_otp(&current_otp);
  
  if(current_otp.infoflag==0xC0)
  {
    rg=current_otp.wb_rg_ratio;
    bg=current_otp.wb_bg_ratio;
    
		r_ratio = 512 * ( S5K4H8_RG_Ratio_Typical) /(rg);
		b_ratio = 512 * ( S5K4H8_BG_Ratio_Typical) /(bg);
	...
	write_cmos_sensor(0xFCFC, 0x4000);
	write_cmos_sensor_8(0x3058, 0x01); //AWB enable
	...
	if(B_GAIN>0x0100)
		...
		LOG_INF("----hejun ofilm  S5K4H8_apply_otp() R_GAIN=%x,B_GAIN=%x,G_GAIN=%x\n",R_GAIN,B_GAIN,G_GAIN);
		/*

int gain_01, gain_02, gain_03, gain_03, gain_04, gain_05, gain_06, gain_07, gain_08;

gain_01 = read_cmos_sensor(0x0210);
gain_02 = read_cmos_sensor(0x0211);
gain_03 = read_cmos_sensor(0x020e);
gain_04 = read_cmos_sensor(0x020f);
gain_05 = read_cmos_sensor(0x0212);
gain_06 = read_cmos_sensor(0x0213);
gain_07 = read_cmos_sensor(0x0214);
gain_08 = read_cmos_sensor(0x0215);

LOG_INF("----hejun ofilm  S5K4H8_apply_otp() R_GAIN=%x,R_GAIN=%x\n",gain_01,gain_02);
LOG_INF("----hejun ofilm  S5K4H8_apply_otp() G_GAIN=%x,G_GAIN=%x\n",gain_03,gain_04);
LOG_INF("----hejun ofilm  S5K4H8_apply_otp() B_GAIN=%x,B_GAIN=%x\n",gain_05,gain_06);
LOG_INF("----hejun ofilm  S5K4H8_apply_otp() Gr_GAIN=%x,Gb_GAIN=%x\n",gain_07,gain_08);
		*/
...
#if 0
static int ovs5k4h8_applyLSC(void)
{
	int i=0,reg1=0,reg2=0;
	LOG_INF("[ovs5k4h8otp]OTP LSC Update Start: \n");
	...
	if(i>=5){
		LOG_INF("[ovs5k4h8otp]OTP LSC Update failed! reg1=%d,reg2=%d,i=%d\n",reg1,reg2,i);
		return -1;
	}
	LOG_INF("[ovs5k4h8otp]OTP LSC  reg1=%d,reg2=%d,i=%d\n",reg1,reg2,i);
	LOG_INF("[ovs5k4h8otp]OTP LSC Update Finished! \n");
	mdelay(5);
...
LOG_INF("[get_imgsensor_id]camIdPin ==  %d \n", camIdPin);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id += 2;
			printk("ovs5k4h8 get_imgsensor_id sensor_id=%x\r\n",*sensor_id);
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
   	...
   	#if OVS5K4H8_OTP_ENABLE
    LOG_INF("[ovs5k4h8otp]read otp start \n");
    if( !g_otpread )
    {
		//write_cmos_sensor(0x0100,0x01);
        if( 0 != s5k4h8_read_otp(0, (u8*)&g_otpdata, sizeof(g_otpdata)) )
        {
            LOG_INF("[ovs5k4h8otp]otp read fail!\n");
            *sensor_id = 0xFFFFFFFF;
            return ERROR_SENSOR_CONNECT_FAIL;
        }
        g_otpread = 1;
    }
    LOG_INF("[ovs5k4h8otp]g_otpdata.module_integrator_id:0x%x, g_otpdata.lens_vcm_id:0x%x, "
	...
	{
        LOG_INF("[ovs5k4h8otp]otp data error!\n");
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
	}

	/**Tarry add for write module info to file start  */
    reg_entry = proc_create(MC_REG_PROC, S_IFREG | S_IRWXUGO, NULL,&mc_reg_fops);
    if (reg_entry == NULL)
    {
        LOG_INF("[ovs5k4h8otp]creat proc %s fail!\n", MC_REG_PROC);
    }
    /**Tarry add for write module info to file end  */
	#endif
	...
	entry = proc_create(MC_PROC, S_IFREG | S_IRWXUGO, NULL,&mc_fops);
	//entry = create_proc_entry(FC_PROC, S_IFREG | S_IRWXUGO, NULL);
	if (entry == NULL)
	{
		LOG_INF("[ovs5k4h8]creat proc %s fail!\n", MC_PROC);
	}
...
static kal_uint32 open(void)
{
	...
	#if OVS5K4H8_OTP_ENABLE
    // ovs5k4h8_applyawb(&g_otpdata);
    //ovs5k4h8_applyLSC();
	S5K4H8_OFilm_apply_otp();
