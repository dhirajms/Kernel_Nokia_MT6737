...
{{ 2592, 1944,	  0,	0, 2592, 1944, 1296,  972, 0000, 0000, 1296,  972,	  0,	0, 1296,  972}, // Preview
 ...
 { 2592, 1944,	  24,  20, 2560, 1920, 1280,  720, 0000, 0000, 1280,  720,	  0,	0, 1280,  720}, // video
...
static void preview_setting(void)
{
	...
	//$MV1[MCLK:24,Width:1296,Height:972,Format:MIPI_Raw10,mipi_lane:2,mipi_datarate:270,pvi_pclk_inverse:0];
	//Streaming off
	write_cmos_sensor(0x0100,0x00);
	//Delay 1 frame
	mDELAY(33);
	//Extclk_frequency_mhz
	write_cmos_sensor(0x0136,0x18); //Extclk_frequency_mhz
	...
	write_cmos_sensor(0x0308,0x43); //pll2_multiplier
	write_cmos_sensor(0x0309,0x82); //op_pix_clk_div
	write_cmos_sensor(0x3C1F,0x00); //reg_pll_s
	write_cmos_sensor(0x3C17,0x02); //reg_pll_s2
	...
	write_cmos_sensor(0x0820,0x01); //requested_link_bit_rate_mbps  270Mbps/lane  ox0344
	write_cmos_sensor(0x0821,0x0E);
	write_cmos_sensor(0x0114,0x01); //CSI_lane_mode
	...
	write_cmos_sensor(0x323b,0x02); //image rotation (only if "flip"  or  "flip + mirror" )
	write_cmos_sensor(0x3301,0x00); //bpc marker (It should be changed by pixel readout orders)
	write_cmos_sensor(0x3321,0x04); //bpc enable (It should be changed by pixel readout orders)
	write_cmos_sensor(0x3306,0x00); //bpmarker_x_addr_start
	write_cmos_sensor(0x3307,0x08);
	write_cmos_sensor(0x3308,0x0A); //bpmarker_x_addr_end
	write_cmos_sensor(0x3309,0x27);
	write_cmos_sensor(0x330A,0x01); //bpmarker_x_odd_inc
	write_cmos_sensor(0x330B,0x01); //bpmarker_x_even_inc
	write_cmos_sensor(0x330E,0x00); //bpmarker_y_addr_start
	write_cmos_sensor(0x330F,0x08);
	write_cmos_sensor(0x3310,0x07); //bpmarker_y_addr_end
	write_cmos_sensor(0x3311,0x9F);
	write_cmos_sensor(0x3312,0x03); //bpmarker_y_odd_inc
	write_cmos_sensor(0x3313,0x01); //bpmarker_y_even_inc
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150){
		...
		write_cmos_sensor(0x323b,0x02); //image rotation (only if "flip"  or  "flip + mirror" )
		write_cmos_sensor(0x3301,0x06); //bpc marker (It should be changed by pixel readout orders)
		write_cmos_sensor(0x3321,0x1C); //bpc enable (It should be changed by pixel readout orders)
		write_cmos_sensor(0x3306,0x00); //bpmarker_x_addr_start
		write_cmos_sensor(0x3307,0x08);
		write_cmos_sensor(0x3308,0x0A); //bpmarker_x_addr_end
		write_cmos_sensor(0x3309,0x27);
		...
		write_cmos_sensor(0x3310,0x07); //bpmarker_y_addr_end
		write_cmos_sensor(0x3311,0x9F);
		write_cmos_sensor(0x3312,0x01); //bpmarker_y_odd_inc
		write_cmos_sensor(0x3313,0x01); //bpmarker_y_even_inc
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x330A,0x01); //bpmarker_x_odd_inc
	write_cmos_sensor(0x330B,0x01); //bpmarker_x_even_inc
	write_cmos_sensor(0x330E,0x01); //bpmarker_y_addr_start
	write_cmos_sensor(0x330F,0x04);
	write_cmos_sensor(0x3310,0x06); //bpmarker_y_addr_end
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor(0x3312,0x07); //bpmarker_y_odd_inc
	write_cmos_sensor(0x3313,0x01); //bpmarker_y_even_inc
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while(imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			printk("s5k5e8mipiraw_Sensor get_imgsensor_id *sensor_id = %x\r\n",*sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("s5k5e8mipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				//return ERROR_NONE;
				break;
			}
			printk("s5k5e8mipiraw_Sensor read sensor id fail, id: 0x%x\n",*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	if (get_otp_module_id(0x04) == 1) {
		printk(KERN_ERR"%s %d *sensor_id = %x module_id = %x \r\n",__func__,__LINE__,*sensor_id,get_otp_module_id(0x04));
	}
	else
	{
		// if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			printk("s5k5e8mipiraw_Sensor open sensor_id = %x\r\n",sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("s5k5e8mipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
...
EXPORT_SYMBOL(S5K5E8_MIPI_RAW_SensorInit);
