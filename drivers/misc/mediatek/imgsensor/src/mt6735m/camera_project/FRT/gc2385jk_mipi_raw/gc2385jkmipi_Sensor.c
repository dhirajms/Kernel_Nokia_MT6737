...
#include "gc2385jkmipi_Sensor.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "[GC2385JK_camera_sensor]"
#define LOG_1 LOG_INF("GC2385JK,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC2385JKDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2385JK_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF("GC2385JKMIPI analogic gain 1x, GC2385JKMIPI add pregain = %d\n",temp);
	}
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 1.43x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 2x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 2.84x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 3.97x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 5.68x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 8.14x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	...
	else if((ANALOG_GAIN_8<= iReg)&&(iReg < ANALOG_GAIN_9))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 11.34x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	...
	else
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385JKMIPI analogic gain 16.23x , GC2385JKMIPI add pregain = %d\n",temp);
	}
	return gain;
...
static kal_uint32 open(void)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC2385JKDuringTestPattern = KAL_FALSE;
...
UINT32 GC2385JK_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
     printk (" GC2385JKMIPI_RAW_SensorInit ()!!!!!!!!!!\n" );
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC2385JK_MIPI_RAW_SensorInit    */
EXPORT_SYMBOL(GC2385JK_MIPI_RAW_SensorInit);
