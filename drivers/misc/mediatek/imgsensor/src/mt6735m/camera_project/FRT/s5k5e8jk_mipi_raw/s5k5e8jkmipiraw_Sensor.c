...
#include "s5k5e8jkmipiraw_Sensor.h"

#define PFX "s5k5e8jkyx_camera_sensor"
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while(imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id += 1;
			printk("s5k5ejk_mipiraw_Sensor get_imgsensor_id *sensor_id = %x\r\n",*sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("s5k5e8jk i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				//return ERROR_NONE;
				break;
			}
			printk("s5k5e8jk read sensor id fail, id: 0x%x\n",*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			sensor_id += 1;
			printk("s5k5e8jkyxmipiraw_Sensor open sensor_id = %x\r\n",sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
...
EXPORT_SYMBOL(S5K5E8JK_MIPI_RAW_SensorInit);
