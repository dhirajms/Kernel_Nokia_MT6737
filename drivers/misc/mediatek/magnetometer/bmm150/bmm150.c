...
//static struct i2c_board_info __initdata bmm150_i2c_info = {I2C_BOARD_INFO(BMM150_DEV_NAME, BMM150_I2C_ADDRESS)};

static int bmm150_init_flag =-1; // 0<==>OK -1 <==> fail
...
static int mag_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	if (!client) {
		mutex_unlock(&bmm150_i2c_mutex);
		return -EINVAL;
	} else if (len > C_I2C_FIFO_SIZE) {
		mutex_unlock(&bmm150_i2c_mutex);
		MSE_LOG(" length %d exceeds %d\n", len, C_I2C_FIFO_SIZE);
		return -EINVAL;
	}
	...
	mutex_unlock(&bmm150_i2c_mutex);
	if (err != 2) {
		MSE_LOG("i2c_transfer error: (%d %p %d) %d\n", addr, data, len, err);
		err = -EIO;
	} else {
		err = 0;
	}
...
static char bmm150_i2c_read_wrapper(u8 dev_addr, u8 reg_addr, u8 *data, u8 len)
{
	...
	return mag_i2c_read_block(this_client, reg_addr, data, len);
}
/*----------------------------------------------------------------------------*/
...
int __attribute__((weak)) bsx_algo_o_get_data(int* x ,int* y,int* z, int* status)
{
	return 0;
}

int __attribute__((weak)) bsx_algo_m_get_data(int* x ,int* y,int* z, int* status)
{
	return 0;
}
...
int bmm150_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ctl.m_enable = bsx_algo_m_enable;
	ctl.m_set_delay  = bsx_algo_m_set_delay;
	ctl.m_open_report_data = bsx_algo_m_open_report_data;
	ctl.o_enable = bsx_algo_o_enable;
	ctl.o_set_delay  = bsx_algo_o_set_delay;
	ctl.o_open_report_data = bsx_algo_o_open_report_data;
	...
	mag_data.get_data_o = bsx_algo_o_get_data;
	mag_data.get_data_m = bsx_algo_m_get_data;
