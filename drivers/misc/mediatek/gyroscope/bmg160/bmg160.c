...
static int bmg_set_opmode(struct i2c_client *client,
		enum BMG_POWERMODE_ENUM power_mode);
...
static int bmg_get_bw(struct i2c_client *client, u8 *bandwidth)
{
	...
	GYRO_ERR("get bw %d, ret %d", *bandwidth, err);
	if (err < 0)
		GYRO_ERR("get bandwidth failed,"
		"err = %d,sensor type = %d\n", err, obj->sensor_type);
				...
				u8 bandwidth)
{
	...
	GYRO_ERR("bmg set bw %d, ret %d", bandwidth, err);
	if (err < 0)
		GYRO_ERR("set bw failed,"
		"err = %d,sensor type = %d\n", err, obj->sensor_type);
...
static ssize_t show_cpsbandwidth_value(struct device_driver *ddri, char *buf)
{
	...
	{
		GYRO_ERR("show cpsbandwidth %d", data);
...
static ssize_t store_cpsbandwidth_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	*/
	GYRO_ERR("set bw %d", (int)bandwidth);
	...
	if (op_mode == 0) {
		...
		mdelay(5);
		error += bmg_set_bw(obj->client, bandwidth);
		GYRO_ERR("bmg_set bw %d, error%d", (int)bandwidth, error);
		mdelay(1);
...

{
   ...
   if (obj->power_mode == BMG_SUSPEND_MODE) {
	...
	{
		GYRO_ERR("fifo frame count is 0!!!");
		return 0;
	}
