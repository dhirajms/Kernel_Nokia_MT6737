...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "CAM_CAL_DRV_S5K4H8"
#define CAM_CAL_I2C_GROUP_ID 0
#define IMGSENSOR_DRVNAME "S5K4H8"
/*******************************************************************************
*
********************************************************************************/
//static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, S5K4H8_OTP_DEVICE_ID>>1)};
...
#define S5K4H8_OTP_INFOFLAG_ADDR           0x0A04
...
S5K4H8_OTP_DATA s5k4h8_otp_data = {0,};
...
#define write_cmos_sensor(addr, para) iWriteReg_otp((u16) addr , (u32) para , 1, S5K4H8_OTP_DEVICE_ID,IMGSENSOR_DRVNAME)
...
int read_s5k4h8_otp_data(void)
{
    ...
    CAM_CALDB("[s5k4h8_otp]read_s5k4h8_otp_data end!!!\n");
	return 0;
}
...
static long s5k4h8otp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case COMPAT_CAM_CALIOC_G_READ:
    {
        ...
        if(err != 0)
            CAM_CALERR("[s5k4h8_otp] compat_put_acdk_sensor_getinfo_struct failed\n");
        return ret;
    }
    default:
        return -ENOIOCTLCMD;
...
static int selective_read_region(u32 offset, BYTE* data, u32 size)
{
    if( !s5k4h8_otp_read )
    {
        CAM_CALDB("[s5k4h8_otp]selective_read_region first read data!!!\n");
        if( 0 != read_s5k4h8_otp_data() )
        {
            CAM_CALDB("[s5k4h8_otp]selective_read_region failed!!!\n");
            return -1;
        }
    }
    CAM_CALDB("[s5k4h8_otp]selective_read_region offset =%x size %d \n", offset,size);
    if( offset == S5K4H8_OTP_INFO_OFFSET+1 && size == 1 )
    {
        CAM_CALDB("[s5k4h8_otp]read module_integrator_id=%d \n", s5k4h8_otp_data.module_integrator_id);
        *data = s5k4h8_otp_data.module_integrator_id;
    }
	else if( offset == S5K4H8_OTP_VCM_OFFSET && size == 1 )
	{
	        *data = 0xDA;
	}
    else if( offset == S5K4H8_OTP_VCM_OFFSET && size == S5K4H8_OTP_VCM_SIZE )
    {
        CAM_CALDB("[s5k4h8_otp]read vcm_inf=%d, vcm_mac=%d \n", s5k4h8_otp_data.vcm_inf, s5k4h8_otp_data.vcm_mac);
        memcpy(data, &s5k4h8_otp_data.vcm_inf, 2);
        memcpy(data+2, &s5k4h8_otp_data.vcm_mac, 2);
    }
    else if( offset == S5K4H8_OTP_INFO_OFFSET && size == sizeof(S5K4H8_OTP_DATA))
    {
        CAM_CALDB("[s5k4h8_otp]read otp info\n");
        memcpy(data, &s5k4h8_otp_data, size);
    }
    else if(size > sizeof(S5K4H8_OTP_DATA))
    {
        CAM_CALDB("[s5k4h8_otp]selective_read_region read size 0x%x is too big! change to max size:0x%x\n", 
                  size, (u32)sizeof(S5K4H8_OTP_DATA));
        size = sizeof(S5K4H8_OTP_DATA);
        memcpy(data, &s5k4h8_otp_data, size);
    }
    else
    {
        CAM_CALDB("[s5k4h8_otp]selective_read_region no offset and size maching!!!\n");
        return -1;
    }
...
int s5k4h8_read_otp(u16 offset, u8* data, u32 size)
{
...
#endif
{
    ...
    else
    {
        ...
        if(NULL == pBuff)
        {
            CAM_CALERR("[s5k4h8_otp]ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                CAM_CALERR("[s5k4h8_otp]ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:
            CAM_CALDB("[s5k4h8_otp]Write CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
            ...
            CAM_CALDB("[s5k4h8_otp]Write data %d bytes take %lu us\n",ptempbuf->u4Length, TimeIntervalUS);
#endif
            break;
        case CAM_CALIOC_G_READ:
            CAM_CALDB("[s5k4h8_otp]Read CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
            do_gettimeofday(&ktv1);
#endif
            ...
            CAM_CALDB("[s5k4h8_otp]Read data %d bytes take %lu us\n",ptempbuf->u4Length, TimeIntervalUS);
#endif
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pu1Params , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pu1Params);
            CAM_CALERR("[s5k4h8_otp]ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    CAM_CALDB("[s5k4h8_otp]CAM_CAL_Open\n");
    spin_lock(&g_CAM_CALLock);
    if(g_u4Opened)
    {
        spin_unlock(&g_CAM_CALLock);
		CAM_CALERR("[s5k4h8_otp]Opened, return -EBUSY\n");
        return -EBUSY;
    }
    else
    {
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    CAM_CALDB("[s5k4h8_otp]RegisterCAM_CALCharDrv\n");
#if CAM_CAL_DYNAMIC_ALLOCATE_DEVNO
    if( alloc_chrdev_region(&g_CAM_CALdevno, 0, 1,CAM_CAL_DRVNAME) )
    {
        CAM_CALERR("[s5k4h8_otp]Allocate device no failed\n");
    ...
    {
        CAM_CALERR("[s5k4h8_otp]Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        CAM_CALERR("[s5k4h8_otp]Allocate mem for kobject failed\n");
    ...
    {
        CAM_CALERR("[s5k4h8_otp]Attatch file operation failed\n");
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "CAM_CALdrv_S5K4H8");
    if (IS_ERR(CAM_CAL_class)) {
        int ret = PTR_ERR(CAM_CAL_class);
        CAM_CALERR("[s5k4h8_otp]Unable to create class, err = %d\n", ret);
        return ret;
    }
...
{
    int i4RetValue = 0;
    CAM_CALDB("[s5k4h8_otp]CAM_CAL_i2C_init\n");
   //Register char driver
	i4RetValue = RegisterCAM_CALCharDrv();
    if(i4RetValue){
 	   CAM_CALDB("[s5k4h8_otp]register char device failed!\n");
	   return i4RetValue;
	}
	CAM_CALDB("[s5k4h8_otp]Attached!! \n");

  //  i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALERR("[s5k4h8_otp]failed to register s5k4h8_otp driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALERR("[s5k4h8_otp]failed to register s5k4h8_otp driver, 2nd time\n");
        return -ENODEV;
    }
