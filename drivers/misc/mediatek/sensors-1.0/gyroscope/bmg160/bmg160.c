...
//static volatile unsigned int I2CDMABuf_pa[DMA_BUFFER_SIZE] = {0};
...
static int bmg_set_opmode(struct i2c_client *client, enum BMG_POWERMODE_ENUM power_mode);

static int gyroscope_local_init(struct platform_device *pdev);
static int gyroscope_remove(void);

static int bmg160_flush(void);
...
#endif
#if 0
static int bmg_i2c_dma_read(struct i2c_client *client, unsigned char regaddr, unsigned char *readbuf, int readlen)
{
    	int ret=0;
	
	if(readlen > DMA_BUFFER_SIZE)
	{
		GYRO_PR_ERR("Read length cann't exceed dma buffer size!\n");
		return -EINVAL;
	}
    	mutex_lock(&i2c_lock);		
	...
	if (ret < 0) {
		GYRO_PR_ERR("dma receive data error!!\n");
		return -EFAULT;
	}
...
static int bmg_set_soft_reset(struct i2c_client *client)
{
	...
	err = bmg_i2c_write_block(client, BMG160_BGW_SOFTRESET_ADDR, &data, 1);
	return err;	
}
...
static int bmg_get_bw(struct i2c_client *client, u8 *bandwidth)
{
	struct bmg_i2c_data *obj = i2c_get_clientdata(client);
	u8 err = 0, data = 0;
	...
	if (obj->sensor_type == BMG160_TYPE) {/* BMG160 */
		...
		*bandwidth = BMG_GET_BITSLICE(data,
			BMG160_BW_ADDR);
	}
	GYRO_PR_ERR("get bw %d, ret %d", *bandwidth, err);
	if (err < 0)
		GYRO_PR_ERR("get bandwidth failed," "err = %d,sensor type = %d\n", err, obj->sensor_type);

	return err;
}

static int bmg_set_bw(struct i2c_client *client, u8 bandwidth)
{
	struct bmg_i2c_data *obj = i2c_get_clientdata(client);
	u8 err = 0, data = 0;
	...
	GYRO_PR_ERR("bmg set bw %d, ret %d", bandwidth, err);
	if (err < 0)
		GYRO_PR_ERR("set bw failed," "err = %d,sensor type = %d\n", err, obj->sensor_type);
...
static int bmg_get_opmode(struct i2c_client *client,
		u8 *op_mode)
{
	//struct bmg_i2c_data *obj = i2c_get_clientdata(client);
	u8 err = 0;
	...
	err += bmg_i2c_read_block(client, BMG160_MODE_LPM1_ADDR, &data1, 1);
	err += bmg_i2c_read_block(client, BMG160_MODE_LPM2_ADDR, &data2, 1);
	data1  = (data1 & 0xA0) >> 5;
	data3  = (data2 & 0x40) >> 6;
	data2  = (data2 & 0x80) >> 7;
	if (data3 == 0x01) {
		*op_mode  = 4;
		...
		if ((data1 == 0x00) && (data2 == 0x00)) {
			*op_mode  = 0;//BMG160_MODE_NORMAL;
		} else {
			if ((data1 == 0x01) || (data1 == 0x05)) {
				*op_mode  = 1;//BMG160_MODE_DEEPSUSPEND;
			} else {
				if ((data1 == 0x04) &&
					(data2 == 0x00)) {
					*op_mode  = 2;//BMG160_MODE_SUSPEND;
				} else {
					if ((data1 == 0x04) &&
						(data2 == 0x01))
						*op_mode  = 3;//BMG160_MODE_FASTPOWERUP;
					}
				}
			}
		}
...
static int bmg_set_opmode(struct i2c_client *client, enum BMG_POWERMODE_ENUM power_mode)
{
	...
	if (obj->sensor_type == BMG160_TYPE) /* BMG160 */
	{
		...
		err = bmg_i2c_read_block(client, BMG160_MODE_LPM1__REG, &data, 1);
		data = BMG_SET_BITSLICE(data, BMG160_MODE_LPM1, actual_power_mode);
		err += bmg_i2c_write_block(client, BMG160_MODE_LPM1__REG, &data, 1);
	...
	if (obj_i2c_data->flush)
	{
		if (obj->power_mode)
		{
			GYRO_LOG("is not flush, will call bmg160_flush in setPowerMode\n");
			bmg160_flush();
		}
		else
		{
		...
		enum BMG_DATARATE_ENUM datarate)
{
	...
	if (obj->sensor_type == BMG160_TYPE) {/* BMG160 */
		if (datarate == BMG_DATARATE_100HZ)
			...
			err = -EINVAL;
			GYRO_PR_ERR("invalid datarate = %d\n", datarate);
			mutex_unlock(&obj->lock);
			return err;
		}
	...
	if (err < 0)
		GYRO_PR_ERR("set bandwidth failed,"
		"err = %d,sensor type = %d\n", err, obj->sensor_type);
	else
		...
		u8 *fifo_mode)
{
	...
	if (err < 0)
		GYRO_PR_ERR("get fifo mode failed,"
		"err = %d,sensor type = %d\n", err, obj->sensor_type);
				...
				u8 fifo_mode)
{
	...
	if (err < 0)
		GYRO_PR_ERR("set fifo mode failed,"
		"err = %d,sensor type = %d\n", err, obj->sensor_type);
		...
		u8 *fifo_framecount)
{
	...
	if (err < 0)
		GYRO_PR_ERR("get fifo framecount failed,"
		"err = %d,sensor type = %d\n", err, obj->sensor_type);
	/*
	else
...
*/
	...
	if (err < 0) {
		GYRO_PR_ERR("get chip type failed, err = %d\n", err);
		return err;
	}
	...
	if (err < 0) {
		GYRO_PR_ERR("set bandwidth failed, err = %d\n", err);
		return err;
	}
	...
	if (0 != reset_cali) {
		...
		if (err < 0) {
			GYRO_PR_ERR("reset calibration failed, err = %d\n", err);
			return err;
		}
	}
		...
		char *buf, int bufsize)
{
	...
	if (err) {
		GYRO_PR_ERR("bmg read raw data failed, err = %d\n", err);
		return -3;
	} else {
		/* compensate data */
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	if (NULL == obj) {
		GYRO_PR_ERR("bmg i2c data pointer is null\n");
		return 0;
	}
		...
		const char *buf, size_t count)
{
	...
	if (1 != sscanf(buf, "%d", &firlen)) {
		GYRO_PR_ERR("invallid format\n");
	} else if (firlen > C_MAX_FIR_LENGTH) {
		GYRO_PR_ERR("exceeds maximum filter length\n");
	} else {
		...
		const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "0x%x", &trace))
		...
		GYRO_PR_ERR("invalid content: '%s', length = %d\n", buf, (int)count);
...
static ssize_t show_op_mode_value(struct device_driver *ddri, char *buf)
{
	ssize_t len = 0;
	struct bmg_i2c_data *obj = obj_i2c_data;
...
static ssize_t store_op_mode_value(struct device_driver *ddri,
		const char *buf, size_t count)
{
	struct bmg_i2c_data *obj = obj_i2c_data;
	...
	if (op_mode == 0) {
		power_mode = BMG_NORMAL_MODE;
	}
	else
		power_mode = BMG_SUSPEND_MODE;

	if (err == 0) {
		err = bmg_set_opmode(obj->client, power_mode);
		if (err)
			return err;
		return count;
	}
...
static ssize_t show_cpsbandwidth_value(struct device_driver *ddri, char *buf)
{
	...
	if (bmg_get_bw(obj->client, &data) < 0)
	{
		return sprintf(buf, "Read error\n");
	}
	else
	{
		GYRO_PR_ERR("show cpsbandwidth %d", data);
...
static ssize_t store_cpsbandwidth_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if ((error = kstrtoul(buf, 10, &bandwidth)))
	{
		return error;
	}
	/*
	...
	*/
	GYRO_PR_ERR("set bw %d", (int)bandwidth);
	error = bmg_get_opmode(obj->client, &op_mode);
	if (op_mode == 0) {
		error += bmg_set_bw(obj->client, bandwidth);
	} else {
		error += bmg_set_opmode(obj->client, (enum BMG_POWERMODE_ENUM)BMG_NORMAL_MODE);
		mdelay(5);
		error += bmg_set_bw(obj->client, bandwidth);
		GYRO_PR_ERR("bmg_set bw %d, error%d", (int)bandwidth, error);
		mdelay(1);
		error += bmg_set_opmode(obj->client, (enum BMG_POWERMODE_ENUM)BMG_SUSPEND_MODE);
		mdelay(3);
	}
/*
	if (bmg_set_bw(obj->client, (unsigned char) data) < 0)
	{
		GYRO_PR_ERR("invalid content: '%s', length = %d\n", buf, (int)count);
	}
*/
...
static ssize_t show_fifo_data_frame_value(struct device_driver *ddri, char *buf)

{
	struct bmg_i2c_data *obj = obj_i2c_data;
	int read_cnt = 0;
    static char tmp_buf[256*6] = {0};
    char *pBuf = NULL;
   ...
   if (obj->power_mode == BMG_SUSPEND_MODE) {
	...
	if (bmg_get_fifo_framecount(obj->client, &fifo_framecount) < 0)
	{
		return sprintf(buf, "Read error\n");
	}
	...
	{
		GYRO_PR_ERR("fifo frame count is 0!!!");
		return 0;
	}

        read_cnt = fifo_framecount;
        pBuf = &tmp_buf[0];
        while (read_cnt > 0)
        {
                if (bmg_i2c_read_block(obj->client,
                        BMG160_FIFO_DATA_ADDR,
                        pBuf,
                        f_len) < 0)
                {
                        GYRO_PR_ERR("[a]fatal error\n");
                        return sprintf(buf, "Read byte block error\n");
                }
                pBuf += f_len;
                read_cnt--;
                //GYRO_PR_ERR("fifo_frame_cnt %d, f_len %d\n", fifo_framecount, f_len);
               memcpy (buf, tmp_buf, fifo_framecount * f_len);
	}
/*
	if(bmg_i2c_dma_read(obj->client, BMG160_FIFO_DATA_ADDR,
		buf, obj->fifo_count * f_len) < 0)
...
static DRIVER_ATTR(cpsopmode, S_IWUSR | S_IRUGO,
		show_op_mode_value, store_op_mode_value);
static DRIVER_ATTR(cpsrange, S_IWUSR | S_IRUGO,
		show_range_value, store_range_value);
static DRIVER_ATTR(cpsbandwidth, S_IWUSR | S_IRUGO,
		show_cpsbandwidth_value,    store_cpsbandwidth_value);
		...
		unsigned long arg)
{
	...
	err = bmg_set_opmode(client, BMG_NORMAL_MODE);
	if (err) {
		err = -EFAULT;
		break;
	}
...
static int bmg_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	if (msg.event == PM_EVENT_SUSPEND) {
		...
		err = bmg_set_opmode(obj->client, BMG_SUSPEND_MODE);
		if (err) {
...
static int bmg160_gyro_enable_nodata(int en)
{
	...
	{
		res = bmg_set_opmode(obj_i2c_data->client, power);
		if(res == 0)
		{
...
static int gyroscope_local_init(struct platform_device *pdev) 
{
	...
	{
		GYRO_PR_ERR("add i2c driver failed\n");
		return -1;
	}
...
static int bmg_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		err = -ENOMEM;
		GYRO_PR_ERR("Allocate DMA I2C Buffer failed! error = %d\n", err);	
	...
	ctl.batch = bmg160_batch;
	ctl.flush = bmg160_flush;
