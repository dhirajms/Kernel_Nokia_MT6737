...
static int wps[7];
static int STK_AUTO_FLAG = -1;
static int ratio_setting = 2115;
...
#ifdef STK_AUTO_CALI
static int32_t stk3x1x_limit_thd_range(int16_t ps_thd_data[]);
static int32_t stk3x1x_auto_set_ps_cali(uint8_t force_cali);
#endif
...
int stk3x1x_write_led(struct i2c_client *client, u8 data)
{
    ...
    ret = stk3x1x_master_send(client, STK_LEDCTRL_REG, &data, 1);
	if(ret < 0)
	{
		APS_ERR("write led = %d\n", ret);
		return -EFAULT;
...
int stk3x1x_write_als(struct i2c_client *client, u8 data)
{
    ...
    ret = stk3x1x_master_send(client, STK_ALSCTRL_REG, &data, 1);
	if(ret < 0)
	{
		APS_ERR("write als = %d\n", ret);
		return -EFAULT;
...
int stk3x1x_read_flag(struct i2c_client *client, u8 *data)
{
	...
	ret = stk3x1x_master_recv(client, STK_FLAG_REG, &buf, 0x01);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
int stk3x1x_write_ps(struct i2c_client *client, u8 data)
{
    ...
    ret = stk3x1x_master_send(client, STK_PSCTRL_REG, &data, 1);
	if (ret < 0)
	{
		APS_ERR("write ps = %d\n", ret);
		return -EFAULT;
...
int stk3x1x_write_wait(struct i2c_client *client, u8 data)
{
    ...
    ret = stk3x1x_master_send(client, STK_WAIT_REG, &data, 1);
	if (ret < 0)
	{
...
int stk3x1x_write_int(struct i2c_client *client, u8 data)
{
    ...
    ret = stk3x1x_master_send(client, STK_INT_REG, &data, 1);
	if (ret < 0)
	{
...
int stk3x1x_write_state(struct i2c_client *client, u8 data)
{
    ...
    ret = stk3x1x_master_send(client, STK_STATE_REG, &data, 1);
	if (ret < 0)
	{
...
int stk3x1x_write_flag(struct i2c_client *client, u8 data)
{
    ...
    ret = stk3x1x_master_send(client, STK_FLAG_REG, &data, 1);
	if (ret < 0)
	{
		APS_ERR("write ps = %d\n", ret);
		return -EFAULT;
...
int stk3x1x_write_sw_reset(struct i2c_client *client)
{
	...
	buf = 0x7F;
    ret = stk3x1x_master_send(client, STK_WAIT_REG, (char*)&buf, sizeof(buf));
	if (ret < 0)
	{
    ...
    ret = stk3x1x_master_recv(client, STK_WAIT_REG, &r_buf, 1);
	if (ret < 0)
	{
	...
	buf = 0;
    ret = stk3x1x_master_send(client, STK_SW_RESET_REG, (char*)&buf, sizeof(buf));
	if (ret < 0)
	{
...
int stk3x1x_write_ps_high_thd(struct i2c_client *client, u16 thd)
{
    ...
    ret = stk3x1x_master_send(client, STK_THDH1_PS_REG, &buf[0], 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %d\n",  ret);
		return -EFAULT;
    ...
    ret = stk3x1x_master_send(client, STK_THDH2_PS_REG, &(buf[1]), 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %d\n", ret);
		return -EFAULT;
...
int stk3x1x_write_ps_low_thd(struct i2c_client *client, u16 thd)
{
    ...
    buf[0] = (u8) ((0xFF00 & thd) >> 8);
    buf[1] = (u8) (0x00FF & thd);	
    ret = stk3x1x_master_send(client, STK_THDL1_PS_REG, &buf[0], 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
    ...
    ret = stk3x1x_master_send(client, STK_THDL2_PS_REG, &(buf[1]), 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
...
int stk3x1x_write_als_high_thd(struct i2c_client *client, u16 thd)
{
    ...
    buf[0] = (u8) ((0xFF00 & thd) >> 8);
    buf[1] = (u8) (0x00FF & thd);	
    ret = stk3x1x_master_send(client, STK_THDH1_ALS_REG, &buf[0], 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
    ...
    ret = stk3x1x_master_send(client, STK_THDH2_ALS_REG, &(buf[1]), 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
...
int stk3x1x_write_als_low_thd(struct i2c_client *client, u16 thd)
{
    ...
    buf[0] = (u8) ((0xFF00 & thd) >> 8);
    buf[1] = (u8) (0x00FF & thd);	
    ret = stk3x1x_master_send(client, STK_THDL1_ALS_REG, &buf[0], 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
    ...
    ret = stk3x1x_master_send(client, STK_THDL2_ALS_REG, &(buf[1]), 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable)
{
	...
	{       
		if (STK_AUTO_FLAG == 1)
		{
			APS_ERR(" Enter stk3x1x_auto_set_ps_cali\n");	
			stk3x1x_auto_set_ps_cali(0);
		}
	}
...
#ifdef STK_AUTO_CALI
static int32_t stk3x1x_get_several_ps_data(int16_t ps_stat_data[])
{
	int err = 0;
	u16 ps_data;
	int8_t data_count = 0;	
	...
	{
		msleep(20);	
		err = stk3x1x_read_ps(stk3x1x_obj->client, &ps_data);

		APS_DBG("%s: ----alex---- ps_data %d=%d\n", __func__, data_count, ps_data);

		if(err != 0)
		{
			APS_DBG("%s: stk3x1x_read_ps fail\n", __func__);
			return -1;
		}
	...
	APS_DBG("%s: ----alex----  %d %d %d\n", __func__, ps_stat_data[STK_DATA_AVE], ps_stat_data[STK_DATA_MAX],ps_stat_data[STK_DATA_MIN]);
...
static int32_t stk3x1x_auto_calc_ct_cali_thd(int16_t ps_stat_data[], int16_t ps_thd_data[])
{
	...
	stk3x1x_limit_thd_range(ps_thd_data);

	APS_DBG("PS_HT=0x%x, PS_LT=0x%x, Ave_CT=0x%x\n", ps_thd_data[STK_HIGH_THD], ps_thd_data[STK_LOW_THD], ps_stat_data[STK_DATA_AVE]);	

    //limit psensor crosstalk by alex
	if((ps_thd_data[STK_HIGH_THD] > STK_MAX_PS_CROSSTALK) || (ps_thd_data[STK_LOW_THD] > STK_MAX_PS_CROSSTALK))
	{
        APS_ERR("Psensor crosstalk is too large!\n");	
...
static void stk3x1x_auto_set_ps_thd_to_driver(int16_t ps_thd_data[])
{
	int ret;

	atomic_set(&stk3x1x_obj->ps_high_thd_val, ps_thd_data[STK_HIGH_THD] ); 
	atomic_set(&stk3x1x_obj->ps_low_thd_val, ps_thd_data[STK_LOW_THD] ); 	

	if((ret = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, ps_thd_data[STK_HIGH_THD])))	
		APS_ERR("%s: write thd failed, err=%d\n", __func__, ret);

	if((ret = stk3x1x_write_ps_low_thd(stk3x1x_obj->client, ps_thd_data[STK_LOW_THD])))	
		APS_ERR("%s: write thd failed, err=%d\n", __func__, ret);		      
}
...
static int32_t stk3x1x_auto_set_ps_cali(uint8_t force_cali)
{
	...
	ret = stk3x1x_get_several_ps_data(ps_statistic_data);
	if(ret < 0)
	{
	...
	ret = stk3x1x_auto_calc_ct_cali_thd(ps_statistic_data, ps_thd_data);
	if(ret < 0)
	{
    ...
    APS_DBG("%s: ---alex--- ps_thd_data[0]=%d, ps_thd_data[1]=%d\n", __func__, ps_thd_data[0],ps_thd_data[1]);

	stk3x1x_auto_set_ps_thd_to_driver(ps_thd_data);	
...
static int32_t stk3x1x_judge_cali_satu(int16_t ps_stat_data[])
{
	...
	if(ps_stat_data[STK_DATA_MAX] < STK_MAX_PS_CROSSTALK)
	{
		...
		APS_DBG("---alex ---: %d %d %d \n", ps_stat_data[STK_DATA_MAX],ps_stat_data[STK_DATA_MIN], diff_ps_data_ct);	

		if (diff_ps_data_ct > STK_MAX_PS_DIFF_AUTO)		
		{
			APS_ERR("Max PS data of crosstalk:%d, Max_ct - Min_ct:%d\n", ps_stat_data[STK_DATA_MAX], diff_ps_data_ct);	
			APS_ERR("The difference between Max PS code and Min PS code is too large\n");
			return -1;
		}
	}
	else
	{
		APS_ERR("Max PS data of crosstalk:%d\n", ps_stat_data[STK_DATA_MAX]);	
		APS_ERR("PS crosstalk is too large\n");
		return -2;				
	}
...
static int32_t stk3x1x_calc_cali_thd(int16_t ps_stat_data[], u16 ps_thd_data[])
{
	ps_thd_data[STK_HIGH_THD] = ps_stat_data[STK_DATA_AVE]*STK_CT_NUMERATOR/STK_CT_DENOMINATOR + STK_THD_H_ABOVE_CT;
	ps_thd_data[STK_LOW_THD] = ps_stat_data[STK_DATA_AVE]*STK_CT_NUMERATOR/STK_CT_DENOMINATOR + STK_THD_L_ABOVE_CT;

	APS_DBG("PS_threshold_high=0x%x\n PS_threshold_low=0x%x\n Max crosstalk=0x%x\n", ps_thd_data[STK_HIGH_THD], ps_thd_data[STK_LOW_THD], ps_stat_data[STK_DATA_MAX]);				
...
static int32_t stk3x1x_set_ps_cali(void)
{
	int16_t ps_statistic_data[3] = {0};
	int32_t ret = 0;
	u16 ps_thd_data[2] = {150, 140};

	ret = stk3x1x_get_several_ps_data(ps_statistic_data);

	APS_DBG("%s:----alex--- ps_value[0]=%d,ps_value[1]=%d,ps_value[2]=%d\n", __func__, ps_statistic_data[0],ps_statistic_data[1],ps_statistic_data[2]);

	if(ret < 0)
	{
		APS_ERR("%s: rpr410_get_several_ps_data failed, ret=%d\n", __func__, ret);

		return ret;
	}

	ret = stk3x1x_judge_cali_satu(ps_statistic_data);
	if(ret == 0)						
	{	
		ret = stk3x1x_calc_cali_thd(ps_statistic_data, ps_thd_data);
		if(ret < 0)
		{
			APS_ERR("%s: calibration fail, errno=%d\n", __func__, ret);	
			wps[0] = stk3x1x_obj->magic_num;
			wps[1] = 0xff;		// calibration fail
			wps[2] = ps_thd_data[STK_HIGH_THD];	// no change
			wps[3] = ps_thd_data[STK_LOW_THD];	// no change
			wps[4] = (int)ps_statistic_data[0];
			wps[5] = (int)ps_statistic_data[1];
			wps[6] = (int)ps_statistic_data[2];
			return ret;
		}

		// write cali result to manufacture data
		wps[0] = stk3x1x_obj->magic_num;
		wps[1] = 0x01;	// calibration ok
		wps[2] = ps_thd_data[STK_HIGH_THD];
		wps[3] = ps_thd_data[STK_LOW_THD];
		wps[4] = (int)ps_statistic_data[0];
		wps[5] = (int)ps_statistic_data[1];
		wps[6] = (int)ps_statistic_data[2];
		printk("PS calibration was done successfully\n");	
...
static ssize_t stk3x1x_show_pscali(struct device_driver *ddri, char *buf)
{
   	ssize_t res;
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_pscali(struct device_driver *ddri, const char *buf, size_t count)
{
	int value;
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_psmanuf(struct device_driver *ddri, char *buf)
{
   	ssize_t res;
	...
	APS_ERR("match_num = %d, cali_status = %d, thdh = %d, thdl = %d, max crosstalk = %d, avg crosstalk = %d, min crosstalk = %d\n", 
			wps[0], wps[1], wps[2], wps[3], wps[4], wps[5], wps[6]);
	res = snprintf(buf, PAGE_SIZE, "%d %d %d %d %d %d %d\n", wps[0], wps[1], wps[2], wps[3], wps[4], wps[5], wps[6]);
...
static DRIVER_ATTR(pscali,  S_IWUSR | S_IRUGO, stk3x1x_show_pscali,   stk3x1x_store_pscali);
static DRIVER_ATTR(psmanuf, S_IWUSR | S_IRUGO, stk3x1x_show_psmanuf, NULL);
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
	...
	als_value = als*ratio_setting/1000;

	//APS_DBG("stk3x1x_get_als_value  %d \n", als);
...
static int als_enable_nodata(int en)
{
	...
	APS_LOG("stk3x1x_obj als enable value = %d, als_flush = %d\n", en, stk3x1x_obj->als_flush);

	if (stk3x1x_obj->als_flush)
	{
		if (en)
		{
			APS_LOG("is not flush, will call stk3x1x_als_flush in als_enable_nodata\n");
			stk3x1x_als_flush();
		}
		else
		{
			stk3x1x_obj->als_flush = false;
		}
	}
	...
	if(en)
	{
		set_bit(CMC_BIT_ALS, &stk3x1x_obj->enable);
	}
	else
	{
		clear_bit(CMC_BIT_ALS, &stk3x1x_obj->enable);
	}
...
static int ps_enable_nodata(int en)
{
	...
	APS_LOG("stk3x1x_obj ps enable value = %d, ps_flush = %d\n", en, stk3x1x_obj->ps_flush);

	if (stk3x1x_obj->ps_flush)
	{
		if (en)
		{
			APS_LOG("is not flush, will call stk3x1x_ps_flush in als_enable_nodata\n");

			stk3x1x_ps_flush();
		}
		else
		{
			stk3x1x_obj->ps_flush = false;
		}
	}
	...
	if(en)
	{
		set_bit(CMC_BIT_PS, &stk3x1x_obj->enable);
	}
	else
	{
		clear_bit(CMC_BIT_PS, &stk3x1x_obj->enable);
	}
...
static int stk3x1x_als_flush(void)
{
	...
	if (!test_bit(CMC_BIT_ALS, &stk3x1x_obj->enable))
	{
		stk3x1x_obj->als_flush = true;
		return 0;
	}
...
static int stk3x1x_ps_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	int value = 0;
...
static int stk3x1x_ps_flush(void)
{
	...
	if (!test_bit(CMC_BIT_PS, &stk3x1x_obj->enable))
	{
		stk3x1x_obj->ps_flush = true;
		return 0;
	}
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	als_ctl.batch = stk3x1x_als_batch;
	als_ctl.flush = stk3x1x_als_flush;
...
static int __init stk3x1x_init(void)
{
	...
	{
		APS_LOG("Enter factory mode\n"); 
	}
	else
	{
		STK_AUTO_FLAG = 1;

		APS_LOG("Enter normal mode\n"); 
	}
